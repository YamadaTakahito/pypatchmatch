...
class PatchMatch:
    ...
    def loadImages(self, files):
        ...
        '''
        self.img = [skimage.img_as_float(skimage.io.imread(
            files[i])).astype(numpy.float32) for i in (0, 1)]
    ...
    def match(self, files, patchSize=(5, 5), iterations=20, minoff=0, maxoff=50):
        ...
        self.nff = numpy.ndarray(
            (self.size[0], self.size[1], 2)).astype(numpy.float32)
        self.cost = numpy.ndarray(
            (self.size[0], self.size[1])).astype(numpy.float32)

        import time
        t = time.time()
        pm.pm(self.img[0], self.img[1], self.nff,
              self.cost, patchSize[0], minoff, maxoff)
        print(time.time() - t)
